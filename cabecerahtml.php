<?php
//print_r($_SESSION);
include_once($folder."class/usuario.php");
$datosusu=new usuario;
$dusuarios=$datosusu->mostrarDatos($_SESSION['CodUsuarioLog']);
$dusuarios=array_shift($dusuarios);
$nombreusuario=$dusuarios['nombre']." ".$dusuarios['paterno'];
?>
<!DOCTYPE html>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0" />
<title>METALDUGAR</title>

<link rel="stylesheet" href="<?php echo $folder?>css/core/style.default.css" type="text/css" />
<link rel="stylesheet" href="<?php echo $folder?>css/general.css" type="text/css" />
<link rel="stylesheet" href="<?php echo $folder?>css/core/responsive-tables.css">
<script type="text/javascript" src="<?php echo $folder?>js/core/jquery-1.9.1.min.js"></script>
<script type="text/javascript" src="<?php echo $folder?>js/core/jquery-migrate-1.1.1.min.js"></script>
<script type="text/javascript" src="<?php echo $folder?>js/core/jquery-ui-1.9.2.min.js"></script>
<script type="text/javascript" src="<?php echo $folder?>js/core/modernizr.min.js"></script>
<script type="text/javascript" src="<?php echo $folder?>js/core/bootstrap.min.js"></script>

<script type="text/javascript" src="<?php echo $folder?>js/core/jquery.cookie.js"></script>
<!--
<script type="text/javascript" src="js/jquery.uniform.min.js"></script>
<script type="text/javascript" src="js/flot/jquery.flot.min.js"></script>
<script type="text/javascript" src="js/flot/jquery.flot.resize.min.js"></script>
<script type="text/javascript" src="js/responsive-tables.js"></script>-->
<script type="text/javascript" src="<?php echo $folder?>js/core/custom.js"></script>
<!--[if lte IE 8]><script language="javascript" type="text/javascript" src="js/excanvas.min.js"></script><![endif]-->